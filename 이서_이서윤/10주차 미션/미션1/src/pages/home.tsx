...
const HomePage = () => {
    ...
    const { data } = useQuery({
    queryFn: () => useGetMovies({ category: "popular", pageParam: 1 , isDetail:false}),
    queryKey: ["movies", "popular"],
    cacheTime: 10000,
    staleTime: 10000,
    });
    const { data:subData } = useQuery({
    queryFn: () => useGetMovies({ category: "now_playing", pageParam: 1 , isDetail:false}),
    ...
    <Container>
        ...
        <SubSwiper>
            {subData?.results?.map((movie:Movie,index:number) => (
                <SwiperSlide key={movie.id}>
                    <SubWrapper>
                        <Img 
